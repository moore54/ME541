...
rc("figure.subplot", left=0.2, bottom=0.18, top=0.97, right=0.92)
...
i2 = 3
m = log10((T2[i2+2]-T2[i2+1])/(T2[i2+3]-T2[i2+2]))/log10(2)
RichExt = T2[i2+3] + (T2[i2+2]-T2[i2+3])/(1-2^m)

PyPlot.close("all")
figname = "HW3_1"
PyPlot.figure(figname)
PyPlot.semilogx(N_cv,T2,".-",label = "Solved Temperature")
PyPlot.semilogx(N_cv[1:3],T2[1:3],"go",label = "Richardson Extrap Pts Order: $(round.(m,3))")
PyPlot.plot(maximum(N_cv),RichExt,"*",label = "Extrapolated Value: $(round.(RichExt,3)) K")
