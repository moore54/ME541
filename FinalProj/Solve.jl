...
for j = 2:length(u_val[:,1])-1
    for i = 2:length(u_val[1,:])-1
        ...
        elseif i==(length(u_val[1,:])-1)
            ...
            Dn = mu/(u_y[j+1]-u_y[j])
            Fn = rho/2*(v_val_star[j,i]+v_val_star[j,i-1])
            An = (Dn + max(-Fn,0.0))*(v_x[i+1]-v_x[i])

            Ds = mu/(u_y[j]-u_y[j-1])
            Fs = rho/2*(v_val_star[j-1,i]+v_val_star[j-1,i-1])
    ...
    for i = 2:length(v_val[1,:])-1
        ...
        elseif i==length(v_val[1,:])-1
            ...
            if j==2 || j==length(v_val[:,1])-1
                ...
                Dn = mu/(v_y[j+1]-v_y[j])
                Fn = rho/2*(v_val_star[j+1,i]+v_val_star[j,i]) #TODO check here if things go astray, j indexing shift back
                An = (Dn + max(-Fn,0.0))*(u_x[i+1]-u_x[i])

                Ds = mu/(v_y[j]-v_y[j-1])
                Fs = rho/2*(v_val_star[j+2,i]+v_val_star[j+1,i])
